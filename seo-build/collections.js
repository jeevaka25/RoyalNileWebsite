...
<div class="inventory-actions"><a class="collection-button" href="${url}">Explore apartment <span aria-hidden="true">↗</span></a><a class="text-link" href="${wa(`Hi! I'd like to check availability for ${v.name}.`)}" target="_blank" rel="noopener" aria-label="Check availability for ${esc(shortName(v))} on WhatsApp">Check dates</a></div></div></article>`;
}
...
<p class="collection-note">These are individual apartment bookings, not exclusive use of an entire villa. Pools are shared. For a group needing several apartments, ask us to check your dates together.</p>
<div class="inventory-grid">${VILLA_ORDER.map((id, index) => villaCard(VILLAS.find(v => v.id === id), index)).join('')}</div></section>
...
function collectionPage(kind, { head, nav, footer }) {
  ...
  return head({ title, desc: description, canonical, ogImage: hero, jsonld, stylesheet: '/collection-pages.css', bodyClass: 'collection-page' }) + `<a class="skip-link" href="#main">Skip to content</a>` + nav(kind) + `<main id="main" class="collection-wrap"><nav class="collection-crumbs" aria-label="Breadcrumb"><a href="/">Home</a><span aria-hidden="true">/</span><span>${villas ? 'Villas & apartments' : 'Tours & experiences'}</span></nav>
...
${villas ? `<div class="collection-trust"><a href="https://www.airbnb.co.uk/users/show/252258998" target="_blank" rel="noopener">Airbnb Superhost <strong>${HOST_TRUST.superhostYears} years</strong></a><span><strong>${HOST_TRUST.rating} ${STAR}</strong> host rating</span><span><strong>${HOST_TRUST.reviews}</strong> Airbnb reviews</span><span class="trust-context">Host-wide credentials</span></div>` : `<div class="collection-trust"><span><strong>09</strong> tours & experiences</span><span><strong>01</strong> eclipse stay</span><span>One place to plan your <strong>Luxor trip</strong></span></div>`}
